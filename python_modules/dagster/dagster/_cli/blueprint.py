...
def _infer_vscode_path() -> Optional[str]:
    base_path = Path.cwd()
...
def configure_vscode(loader_module: Optional[str] = None, vscode_folder_path: Optional[str] = None):
    ...
    if loader_module:
        ...
        origins = PyProjectFileTarget("pyproject.toml").create_origins()
        for origin in origins:
            if isinstance(origin, ManagedGrpcPythonEnvCodeLocationOrigin):
                module = origin.loadable_target_origin.module_name
                if module:
                    loaders += load_blueprints_loaders_from_module_path(module)
...
def recommend_yaml_extension():
    ...
    extensions = subprocess.check_output(["code", "--list-extensions"]).decode("utf-8").split("\n")
    if "redhat.vscode-yaml" in extensions:
        ...
        if click.confirm(
            ...
            subprocess.check_call(["code", "--install-extension", "redhat.vscode-yaml"])
...
def install_yaml_schema_extension(dot_vscode_path: Path, yaml_dir: Path, schema_path: Path) -> None:
    ...
    try:
        subprocess.check_call(["code", "--uninstall-extension", "dagster.dagster-blueprint-schema"])
    except subprocess.CalledProcessError:
        click.echo("No existing dagster.dagster-blueprint-schema extension to uninstall.")
    subprocess.check_call(["code", "--install-extension", os.fspath(extension_zip_path.resolve())])
